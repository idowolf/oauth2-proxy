...
func validateAllowlists(o *options.Options) []string {
	...
	msgs = append(msgs, validateCSRFRoutes(o)...)
...
// validateCSRFRoutes validates method=path routes passed with options.SkipCSRFRoutes
func validateCSRFRoutes(o *options.Options) []string {
	msgs := []string{}
	for _, route := range o.SkipCSRFRoutes {
		var regex string
		parts := strings.SplitN(route, "=", 2)
		if len(parts) == 1 {
			regex = parts[0]
		...
		_, err := regexp.Compile(regex)
		if err != nil {
			msgs = append(msgs, fmt.Sprintf("error compiling CSRF skip route regex /%s/: %v", regex, err))
		}
	}
	return msgs
}
